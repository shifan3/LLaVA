...
export OUTPUT_DIR=./checkpoints/llava-$MODEL_NAME-formula

rm $OUTPUT_DIR/checkpoint-* -rf

deepspeed llava/train/train_mem.py \
    --deepspeed ./scripts/zero2.json \
    --model_name_or_path /root/mathlens_2.0/models/formula-zh-$MODEL_NAME/model \
    --version v1 \
    --data_path data/formula/formula_finetune.train.json \
    --image_folder data/formula/images \
    --vision_tower $CLIP_MODEL \
    --pretrain_mm_mlp_adapter ./checkpoints/llava-$MODEL_NAME-clip-vit-large-patch14-336-chn-pretrain/mm_projector.bin \
